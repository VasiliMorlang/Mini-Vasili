...
export default function App() {
  ...
  const loadDrive = async () => {
    ...
    try {
      ...
      setDriveEntries(data.map((d, i) => ({ ...d, id: "drive-" + i, isFile: true })));
  ...
  const allEntries = [...driveEntries, ...entries];

  const buildSystem = () => {
    if (!allEntries.length) return "Du bist Mini-Vasili. Deine Wissensbasis ist noch leer. Sag dem Nutzer das locker und mit Humor. Antworte auf Deutsch.";
        ...
        <aside className="sidebar">
          ...
          <nav>
            ...
            <button className={`nav-item ${view === "admin" ? "active" : ""}`} onClick={() => setView("admin")}>
              ⚙️ Wissensbasis {allEntries.length > 0 && <span className="nav-badge">{allEntries.length}</span>}
            </button>
          </nav>
          <div className="sidebar-foot">{allEntries.length === 0 ? "⚠️ Keine Daten gepflegt" : `✅ ${allEntries.length} Einträge aktiv`}</div>
        </aside>
        <main className="main">
          {view === "admin" && (
            <div className="admin">
              <div className="page-head" style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between"}}>
                <div><h2>Wissensbasis verwalten</h2><p>Google Drive Ordner + manuelle Einträge.</p></div>
                <button className="btn-blue" onClick={loadDrive} style={{marginTop:4,display:"flex",alignItems:"center",gap:6}}>
                  {driveLoading ? "⏳ Lädt…" : "🔄 Drive aktualisieren"}
                </button>
              </div>
              {driveEntries.length > 0 && (
                <div className="card">
                  <div className="card-title">📂 Aus Google Drive geladen ({driveEntries.length})</div>
                  <div className="entry-list">
                    {driveEntries.map(e => (
                      <div className="entry" key={e.id}>
                        <div className="entry-l"><span className="entry-ico">📄</span><div style={{minWidth:0}}><div className="entry-name">{e.title}</div><div className="entry-meta">{e.content.length.toLocaleString("de")} Zeichen</div></div></div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {view === "chat" && (
            <div className="chat">
              <div className="chat-head">
                ...
                <div><div className="chat-hname">Mini-Vasili</div><div className="chat-status">● Online · {allEntries.length} Infoquellen</div></div>
                <button className="btn-ghost" onClick={resetChat}>↺ Neu starten</button>
              </div>
              {allEntries.length === 0 && !driveLoading && <div className="kb-warn">⚠️ Wissensbasis leer – gehe zu <strong>Wissensbasis</strong> und füge Infos hinzu.</div>}
